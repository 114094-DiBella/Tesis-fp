    ...
    public class PaymentController {
        ...
        // Agregar en PaymentController
        @PostMapping("/debug-preference")
        public ResponseEntity<?> debugPreference(@Valid @RequestBody PaymentRequest request) {
            try {
                logger.info("=== DEBUG PREFERENCE REQUEST ===");
                logger.info("Request: {}", request);

                // Mostrar exactamente qué se va a enviar a MP
                Map<String, Object> debugInfo = new HashMap<>();
                debugInfo.put("orderCode", request.getOrderCode());
                debugInfo.put("amount", request.getAmount());
                debugInfo.put("productName", request.getProductName());
                debugInfo.put("description", request.getDescription());
                debugInfo.put("quantity", request.getQuantity());
                debugInfo.put("baseUrl", "https://3c43-190-247-133-65.ngrok-free.app");
                debugInfo.put("webhookUrl", "https://3c43-190-247-133-65.ngrok-free.app/api/payments/webhook");
